    ...
    <div className="mx-auto max-w-md px-4 py-6">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        {GAMES.map((g, i) => {
          const inner = (
            <div
              className={`glass flex aspect-square flex-col items-center justify-center gap-3 rounded-2xl p-4 transition-transform ${g.unlocked ? "hover:scale-[1.02]" : "opacity-60"}`}
            ...
            >
              <div className="grid h-16 w-16 place-items-center rounded-full" style={{ background: `${g.color}22`, border: `1px solid ${g.color}55` }}>
                {g.unlocked ? <Gamepad2 className="h-7 w-7" style={{ color: g.color }} /> : <Lock className="h-6 w-6 text-muted-foreground" />}
              </div>
          ...
          return g.unlocked && g.to ? (
            <Link key={g.name} to={g.to}>{inner}</Link>
          ) : (
